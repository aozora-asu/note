...
<!--
日本のEVの現状として、未だ日本はEV保有台数は全自動の1%以下であり、送配電ネットワークへの影響は限定的です。
日本では、2035年までにEV100%を目標としています。
EV割合が増えていくにつれて、送配電ネットワークへの影響が現れ始めます。
ですので、送配電ネットワークへの影響を緩和し、送配電ネットワークと統合する方策が必要となってきます。
これは、EVの普及に応じたEVの系統統合の枠組みです
Phase1 
...
また、充電器の設置は地域の配電ネットワークにも依存するところがあります
...
<!--
ピーク時間帯でのEV充電を避ける方法を模索するために、研究会では、いくつかの需要家のサンプルについての充電パターンが紹介さ
れてました
...
<!--
EVユーザーの充電タイミングをコントロールする方法についてお話しします。大きく分けて2つの方法があります。
...
<!--
「国別の対応として、アメリカの対応が紹介されていました。英国では、夜間料金プランとスマート充電規制が効果を上げています。

夜間料金プランでは、通常料金の70%も割安な料金を提供しています。たとえば、1kWhあたり0.26ポンドが、夜間には0.075ポンドになるというものです。これにより、夜間の安価な電力を活用し、顧客負担を削減しています。そして夜間に充電することのモチベーションを惹起するようにしてます。
...
次に、日本の対応についてです。現在、日本では電気の使用者が時間帯別料金を利用するために、使用時間と使用電力量を
記録できる、いわゆるスマートメーターの導入を推進しており、2024年度末までに全数設置を目指しています。これにより、時間帯別料金の利用が可能になり、ユーザーの行動を夜間充電に誘導できます。
...
短期的には、充電インフラ整備と夜間料金の普及が鍵となります。中期的には、スマート充電規制の義務化が重要です。そして、長期的にはV2Gの普及を進め、需給調整市場を構築する必要があります。
